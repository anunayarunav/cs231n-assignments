...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dh, dw2, db2 = affine_backward(dx, cache2)
    _, dw1, db1 = affine_relu_backward(dh, cache1)
...
class FullyConnectedNet(object):
  ...
  def affine_bn_relu_dropout_forward(self, x, w, b, gamma, beta, bn_param, dropout_param):
  ...
  def affine_bn_relu_dropout_backward(self, dout, cache):
    ...
    dx, dw, db = affine_backward(dx, aff_cache)
    return dx, dw, db, dgamma, dbeta
  
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
               dropout=0, use_batchnorm=False, reg=0.0,
               weight_scale=1e-2, dtype=np.float32, use_svm=False,plugin=False, seed=None):
    """
    ...
    for i in xrange(self.num_layers-1):
      d2 = hidden_dims[i]
      self.params['W%d'%(i+1)] = weight_scale*np.random.randn(d1, d2)
      self.params['b%d'%(i+1)] = np.zeros(d2)
      if self.use_batchnorm:
         self.params['gamma%d'%(i+1)] = np.ones(d2)
         self.params['beta%d'%(i+1)] = np.zeros(d2)
      
      d1 = d2
    
    self.params['W%d'%self.num_layers] = weight_scale*np.random.randn(d2, num_classes)
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(self.num_layers-1):
      ...
      if self.use_batchnorm or self.use_dropout:
          out, cache[i+1] = self.affine_bn_relu_dropout_forward(out, w, b, gamma, beta, bn_param, dropout_param)
      else:
          out, cache[i+1] = affine_relu_forward(out, w, b)

    W, b = self.params['W%d'%self.num_layers], self.params['b%d'%self.num_layers]
    out, cache[self.num_layers] = affine_forward(out, W, b)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx, dw, db = affine_backward(dx, cache[self.num_layers])
    dw += reg*W
    ...
    for i in xrange(self.num_layers-1, 0, -1):
        if self.use_batchnorm or self.use_dropout:
            dx, dw, db, dgamma, dbeta = self.affine_bn_relu_dropout_backward(dx, cache[i])
        else:
